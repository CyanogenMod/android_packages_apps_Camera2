...
public class PhotoMenu extends PieController
    ...
    public void initialize(PreferenceGroup group) {
        ...
        // ISO
        final ListPreference isoPref = group.findPreference(CameraSettings.KEY_ISO);
        if (isoPref != null) {
            item = makeListItem(CameraSettings.KEY_ISO);
            item.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(PieItem item) {
                    showListPopup(isoPref);
                }
            });
            advanced.addItem(item);
        }
        ...
        if (burstPref != null) {
            item = makeItem(R.drawable.ic_burst);
            item.setLabel(res.getString(R.string.pref_camera_burst_title).toUpperCase(locale));
            more.addItem(item);
